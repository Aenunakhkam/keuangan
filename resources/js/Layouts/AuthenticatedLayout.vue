...
<template>
    <div class="min-h-screen bg-gray-50 dark:bg-gray-950 font-sans antialiased">
        ...
        <nav class="bg-[#166534] text-white shadow-lg sticky top-0 z-50">
                    ...
                    <div class="flex items-center space-x-4">
                        ...
                        <div class="flex shrink-0 items-center bg-white p-2 rounded-xl shadow-sm relative z-10">
                            <Link :href="route('dashboard')">
                                ...
                                <ApplicationLogo v-else class="block h-10 w-auto fill-current text-[#166534]" />
                            </Link>
                        </div>
                        <!-- App Name -->
                        <div class="hidden sm:block relative z-10">
                            <div class="text-xs font-medium text-green-200 uppercase tracking-widest leading-tight">
                            ...
                            <Dropdown align="right" width="48">
                                <template #trigger>
                                    <button type="button" class="flex items-center space-x-3 p-1.5 rounded-full hover:bg-white/10 transition-colors duration-200 border border-white/10">
                                        <div class="h-9 w-9 bg-gray-200 rounded-full overflow-hidden flex-shrink-0 border-2 border-green-400">
                ...
                <div class="hidden lg:flex border-t border-white/10 pb-0.5">
                    <div class="flex space-x-1 py-1">
                        ...
                        <Link :href="route('dashboard')" 
                            ...
                            :class="route().current('dashboard') ? 'bg-white/10 text-white shadow-sm ring-1 ring-white/20' : 'text-green-100 hover:bg-white/10 hover:text-white'">
            ...
            <div v-show="showingNavigationDropdown" class="fixed inset-0 z-[100] flex lg:hidden" aria-modal="true" role="dialog">
                ...
                <div class="relative flex w-full max-w-xs flex-1 flex-col bg-white h-full shadow-2xl overflow-y-auto transform transition-transform duration-300">
                    ...
                    <div class="flex items-center space-x-4 px-6 py-6 border-b border-gray-100">
                        ...
                        <ApplicationLogo v-else class="h-12 w-auto text-[#166534]" />
                        <div>
                            <div class="text-[10px] text-gray-400 font-medium uppercase tracking-widest">{{ $page.props.settings?.app_name || 'SIM Akademik' }}</div>
                            <div class="text-sm font-black text-[#166534]">{{ $page.props.settings?.school_name || 'Universitas Harkat Negeri' }}</div>
                        </div>
                    </div>
                    ...
                    <div class="px-6 py-4 bg-gray-50 border-b border-gray-100 flex items-center justify-between">
                        <div class="flex items-center space-x-3">
                            <div class="h-10 w-10 bg-green-100 rounded-full flex items-center justify-center font-bold text-green-700">
                                {{ $page.props.auth.user.name.charAt(0) }}
                            </div>
                            <div>
                                <div class="text-sm font-bold text-gray-900">{{ $page.props.auth.user.name }}</div>
                                <Link :href="route('profile.edit')" class="text-xs text-green-600 font-medium hover:underline">Edit Profil</Link>
                            </div>
                        </div>
                    ...
                    <nav class="flex-1 overflow-y-auto w-full">
                        <div class="flex flex-col w-full">
                            ...
                            <!-- Beranda / Dashboard -->
                            <Link :href="route('dashboard')" class="w-full flex items-center space-x-3 text-left px-6 py-4 border-b border-gray-100 text-sm font-medium transition-colors" :class="route().current('dashboard') ? 'text-green-600 bg-green-50/50' : 'text-gray-700 hover:bg-gray-50'">
                                <svg class="h-5 w-5" :class="route().current('dashboard') ? 'text-green-600' : 'text-gray-400'" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6" /></svg>
                                <span>Beranda</span>
                            </Link>
                            ...
                            <div class="w-full border-b border-gray-100" v-if="$page.props.auth.user.roles.some(r => ['admin', 'bendahara'].includes(r))">
                                ...
                                >
                                    <div v-show="activeMobileMenu === 'keuangan'" class="bg-gray-50/50 pb-2">
                                        ...
                                        <Link :href="route('cash-transactions.index')" class="flex items-center space-x-3 w-full px-10 py-3 text-sm text-gray-600 hover:text-green-600 hover:bg-white transition-colors" :class="{'text-green-600 font-semibold bg-white': route().current('cash-transactions.*')}">
                                            <svg class="h-4 w-4" :class="route().current('cash-transactions.*') ? 'text-green-600' : 'text-gray-400'" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7v8a2 2 0 002 2h6M8 7V5a2 2 0 012-2h4.586a1 1 0 01.707.293l4.414 4.414a1 1 0 01.293.707V15a2 2 0 01-2 2h-2M8 7H6a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2v-2" /></svg>
                                            <span>Jurnal Kas</span>
                                        </Link>
                                    </div>
                                </transition>
                             ...
                             <div class="w-full border-b border-gray-100" v-if="$page.props.auth.user.roles.some(r => ['admin', 'bendahara'].includes(r))">
                                 ...
                                 >
                                     <div v-show="activeMobileMenu === 'laporan'" class="bg-gray-50/50 pb-2">
                                         <Link :href="route('reports.index')" class="flex items-center space-x-3 w-full px-10 py-3 text-sm text-gray-600 hover:text-green-600 hover:bg-white transition-colors" :class="{'text-green-600 font-semibold bg-white': route().current('reports.index')}">
                                             <svg class="h-4 w-4" :class="route().current('reports.index') ? 'text-green-600' : 'text-gray-400'" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" /></svg>
                                             <span>Ringkasan Keuangan</span>
                                         </Link>
                                         <Link :href="route('reports.transactions')" class="flex items-center space-x-3 w-full px-10 py-3 text-sm text-gray-600 hover:text-green-600 hover:bg-white transition-colors" :class="{'text-green-600 font-semibold bg-white': route().current('reports.transactions')}">
                                             <svg class="h-4 w-4" :class="route().current('reports.transactions') ? 'text-green-600' : 'text-gray-400'" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                                             <span>Laporan Transaksi</span>
                                         </Link>
                                     </div>
                                 </transition>
                            ...
                            <div class="w-full border-b border-gray-100" v-if="$page.props.auth.user.roles.includes('admin')">
                                ...
                                >
                                    <div v-show="activeMobileMenu === 'pengaturan'" class="bg-gray-50/50 pb-2">
                                        <Link :href="route('settings.index')" class="flex items-center space-x-3 w-full px-10 py-3 text-sm text-gray-600 hover:text-green-600 hover:bg-white transition-colors" :class="{'text-green-600 font-semibold bg-white': route().current('settings.index')}">
                                            <svg class="h-4 w-4" :class="route().current('settings.index') ? 'text-green-600' : 'text-gray-400'" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" /></svg>
                                            <span>Pengaturan Umum</span>
                                        </Link>
                                        <Link :href="route('users.index')" class="flex items-center space-x-3 w-full px-10 py-3 text-sm text-gray-600 hover:text-green-600 hover:bg-white transition-colors" :class="{'text-green-600 font-semibold bg-white': route().current('users.index')}">
                                            <svg class="h-4 w-4" :class="route().current('users.index') ? 'text-green-600' : 'text-gray-400'" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" /></svg>
                                            <span>Kelola Akun</span>
                                        </Link>
                                    </div>
                                </transition>
